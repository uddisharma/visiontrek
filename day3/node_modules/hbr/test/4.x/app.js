...
hbr.registerHelper('make_error', function () {
  throw new TypeError('oops!')
})

hbr.registerHelper('link_to', function(context) {
  return "<a href='" + context.url + "'>" + context.body + "</a>";
});

hbr.registerHelper('link_to2', function(title, context) {
  return "<a href='/posts" + context.url + "'>" + title + "</a>"
});

hbr.registerHelper('list', function(items, context) {
  var out = "<ul>";
...
hbr.registerPartial('link2', '<a href="/people/{{id}}">{{name}}</a>');
hbr.registerPartials(__dirname + '/views/partials');
...
before(function () {
  ...
  // render html files using hbr as well
  // tests detecting the view engine extension
  app.engine('html', hbr.__express)
...
test('helper error', function (done) {
  request(app)
    .get('/helper-error')
    .expect(500)
    .expect(shouldHaveFirstLineEqual('TypeError: ' + path.join(__dirname, 'views', 'error.hbr') + ': oops!'))
    .end(done)
})
...
test('syntax error', function(done) {
  request(app)
    .get('/syntax-error')
    .expect(500)
    .expect(shouldHaveFirstLineEqual('Error: ' + path.join(__dirname, 'views', 'syntax-error.hbr') + ': Parse error on line 1:'))
    .end(done)
});
...
test('syntax error layout', function (done) {
  request(app)
    ...
    .expect(500)
    .expect(shouldHaveFirstLineEqual('Error: ' + path.join(__dirname, 'views', 'bad_layout.hbr') + ': Parse error on line 3:'))
    .end(done)
})
